...
2. **Why FMU for Slip?** Standard kinematic simulations in NX MCD assume "infinite friction" (parts stick together perfectly). Integrating an FMU allows for the simulation of **failure modes**, proving the robustness of the control logic against aggressive tuning parameters.
   ...
   [Wafer aligner](https://www.3dfindit.com/en/digitaltwin/?path=jel%2Faligner.prj&mident={jel%2Faligner.prj}%2C013+{LINEID%3D80}++{NB%3DSAL46C6}%2C{ORDERNO%3DSAL46C6}%2C{CARRYING_OBJECT%3DSEMI+standard+150+to+300mm+silicon+wafer}%2C{CLEANLINESS%3D0.1%C2%B5mcf+Class+1+(at+wafer+transfer+level+when+exhausting+driving+part)}%2C{DRIVING_METHOD%3D2-phase+stepping+motor+(for+4+axes)internal+motor+driver%2C+controller}%2C{POSITIONING_ACCURACY%3DCentering+Within+%C2%B10.1+mmFlat+locatingNotch+locating+Within+%C2%B10.1+deg}%2C{POSITIONING_TIME%3DCentering+3+sec}%2C{SENSOR%3DVisible+laser+sensor}%2C{UTILITY%3DPower+DC24V%C2%B110%25%C2%A04A%3B+Vacuum+-53+kPa+or+more}%2C{NAME%3DSAL46C6}%2C{SOURCEURL%3Dhttps%3A%2F%2Fwww.jel-robot.com%2Fproducts%2FSAL46C6_a.html}%2C{WAFER_SIZE_CHANGE%3DBy+command+control+or+switch})
